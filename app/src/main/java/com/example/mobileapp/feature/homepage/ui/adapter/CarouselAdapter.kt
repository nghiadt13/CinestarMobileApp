...
class CarouselAdapter(
    ...
    inner class CarouselViewHolder(private val binding: ItemBannerImageBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: CarouselItem) {
            Glide.with(binding.imageViewBanner.context)
                .load(item.imageUrl)
                .placeholder(R.drawable.placeholder)
                .error(R.drawable.error)
                .transition(DrawableTransitionOptions.withCrossFade(4))
